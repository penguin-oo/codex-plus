...
public final class AccountCenterPresentation {
    ...
    public static String backendSummary(
        ...
        if (backend.isTokenPoolMode() && !isBlank(backend.currentTokenFile)) {
            summary.append("\nCurrent token: ").append(backend.currentTokenFile.trim());
            if (!isBlank(backend.currentTokenEmail)) {
                summary.append("\nCurrent token email: ").append(backend.currentTokenEmail.trim());
            }
            if (!isBlank(backend.currentTokenPlanType)) {
                summary.append("\nCurrent token plan: ").append(backend.currentTokenPlanType.trim());
            }
            if (!isBlank(backend.currentTokenQuotaSummary)) {
                summary.append("\nCurrent token quota: ").append(backend.currentTokenQuotaSummary.trim());
            }
        }
