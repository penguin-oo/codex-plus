...
public final class PortalApiClient {
    ...
    public PortalBootstrap fetchBootstrap(PortalEndpoint endpoint) throws IOException {
        ...
        return new PortalBootstrap(
                ...
                parseStringList(json.optJSONArray("default_portal_urls")),
                json.optString("remote_restart_target_label")
        );
    }
    ...
    public BackendStatusPayload saveBackendStatus(
        ...
        return saveBackendStatus(endpoint, backendMode, tokenDir, proxyPort, openaiBaseUrl, openaiApiKey, openaiModel, "", "", "");
    }
    ...
    public BackendStatusPayload applyBackendPreset(PortalEndpoint endpoint, String presetId) throws IOException {
        JSONObject json = postJson(endpoint, "/api/backend", buildApplyBackendPresetBody(presetId));
        return parseBackendStatus(json);
    }
    ...
    public void restartRemoteComputer(PortalEndpoint endpoint, String host, String user, String password) throws IOException {
        JSONObject body = new JSONObject();
        try {
            body.put("host", host);
            body.put("user", user);
            ...
            throw new IOException("Failed to build remote restart request.", exception);
        }
        postJson(endpoint, "/api/remote/restart", body);
    }

    public void restartLocalComputer(PortalEndpoint endpoint) throws IOException {
        postJson(endpoint, "/api/desktop/restart", new JSONObject());
    }
    ...
    public JSONObject clearSessionGoalContext(PortalEndpoint endpoint, String sessionId) throws IOException {
        return postJson(endpoint, "/api/sessions/" + sessionId + "/goal/clear", new JSONObject());
    }
    ...
    static BackendStatusPayload parseBackendStatus(JSONObject json) {
        JSONObject currentTokenQuota = json.optJSONObject("current_token_quota");
        return new BackendStatusPayload(
                ...
                json.optString("openai_api_key"),
                json.optBoolean("has_openai_api_key", false),
                parseStringList(json.optJSONArray("openai_models")),
                parseOpenAiPresets(json.optJSONArray("openai_presets")),
                json.optString("active_openai_preset_id"),
                json.optString("proxy_preference", "direct"),
                json.optString("last_error"),
                currentTokenQuota == null ? "" : currentTokenQuota.optString("token_file"),
                currentTokenQuota == null ? "" : currentTokenQuota.optString("email"),
                currentTokenQuota == null ? "" : currentTokenQuota.optString("plan_type"),
                currentTokenQuota == null ? "" : currentTokenQuota.optString("summary")
        );
    }
    ...
    static JSONObject buildBackendSettingsBody(
        ...
        return buildBackendSettingsBody(backendMode, tokenDir, proxyPort, openaiBaseUrl, openaiApiKey, openaiModel, "", "", "");
    }
    ...
    private static List<OpenAiPresetSummary> parseOpenAiPresets(JSONArray array) {
        List<OpenAiPresetSummary> presets = new ArrayList<>();
        if (array == null) {
            return presets;
        }
        for (int index = 0; index < array.length(); index++) {
            ...
            presets.add(new OpenAiPresetSummary(
                    item.optString("id"),
                    item.optString("name"),
                    item.optString("openai_base_url"),
                    item.optString("openai_model"),
                    parseStringList(item.optJSONArray("openai_models")),
                    item.optString("openai_protocol"),
                    item.optString("openai_api_key"),
                    item.optString("proxy_preference", "direct"),
                    item.optBoolean("has_openai_api_key", false)
            ));
        }
        return presets;
    }
