...
public final class PortalConfigStore {
    ...
    public void replaceRecentPortalUrls(List<String> portalUrls) {
        ...
        if (portalUrls != null) {
            for (String portalUrl : portalUrls) {
                ...
                if (filtered.contains(portalUrl)) {
                    continue;
                }
                filtered.add(portalUrl);
                if (filtered.size() >= MAX_RECENT_PORTAL_URLS) {
                    break;
                }
            }
        }
        preferences.edit().putString(KEY_RECENT_PORTAL_URLS, encodeList(filtered)).apply();
    }
