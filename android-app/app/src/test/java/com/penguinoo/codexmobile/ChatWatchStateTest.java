...
public final class ChatWatchStateTest {
    ...
    @Test
    public void displayError_usesJobErrorWhenPresent() {
        PortalJob job = new PortalJob("job-1", "failed", "session-1", "", "upstream 403", "", 0, "mobile", "Mobile");

        assertEquals("upstream 403", ChatWatchState.displayError(job));
    }

    @Test
    public void displayError_usesGenericErrorWhenMissing() {
        PortalJob job = new PortalJob("job-1", "failed", "session-1", "", "", "", 0, "mobile", "Mobile");

        assertEquals("Job failed.", ChatWatchState.displayError(job));
    }
}
