...
public final class ChatConversationStateTest {
    ...
    @Test
    public void compose_canRenderFinishedLiveTextWithoutReplyingStatus() {
        List<ChatMessage> persisted = new ArrayList<>();
        persisted.add(new ChatMessage("user", "hello", 1L));
        ...
        ChatMessage last = display.get(display.size() - 1);
        assertEquals("partial answer before failure", last.text);
        assertEquals("assistant", last.role);
        assertFalse(last.isEphemeral);
    }
