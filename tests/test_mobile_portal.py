...
class ProxySettingsTests(unittest.TestCase):
                ...
                json.dumps(
                    {
                        ...
                        "public_urls": [
                            ...
                            "https://codex.example.com/?token=stale#fragment",
                            "https://codex.example.com/path?foo=1",
                            "not-a-url",
                            "",
                        ],
                    }
    ...
    def test_load_proxy_settings_accepts_utf8_bom_files(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            payload = json.dumps(
                {
                    "proxy_enabled": True,
                    "proxy_port": 7897,
                    "public_urls": ["https://codex.example.com"],
                }
            ).encode("utf-8")
            ...
            self.assertEqual(["https://codex.example.com/"], settings["public_urls"])
    ...
    def test_build_codex_subprocess_env_uses_saved_settings_over_existing_proxy_env(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with mock.patch.dict(
                ...
                env = mobile_portal.build_codex_subprocess_env(settings_file=settings_path, backend_settings_file=backend_settings_path)
...
class JobRunnerOwnershipTests(unittest.TestCase):
    ...
    def test_active_job_for_session_recovers_completed_jsonl_turn_even_when_pid_is_alive(self) -> None:
        runner = mobile_portal.JobRunner(_FakeStore())
        runner.active_sessions.add("session-1")
        runner.jobs["job-1"] = {
            "job_id": "job-1",
        ...
        runner.data_store.latest_task_complete_message = mock.Mock(return_value=(250, "final reply"))

        with mock.patch.object(runner, "_is_pid_running", return_value=True), \
        ...
        self.assertEqual("completed", runner.jobs["job-1"]["status"])
        self.assertEqual("final reply", runner.jobs["job-1"]["last_message"])
        self.assertTrue(runner.jobs["job-1"]["has_final_answer"])
        self.assertEqual(0, runner.jobs["job-1"]["pid"])
        terminate_pid.assert_called_once_with(1744)
    ...
    def test_latest_failed_job_for_session_returns_recent_failed_job_payload(self) -> None:
        runner = mobile_portal.JobRunner(_FakeStore())
        runner.jobs["old-failed"] = {
            "job_id": "old-failed",
            "status": "failed",
            "kind": "resume",
            "session_id": "session-1",
        ...
        job = runner.latest_failed_job_for_session("session-1")

        self.assertIsNotNone(job)
        self.assertEqual("new-failed", job["job_id"])
        self.assertEqual("new error", job["error"])

    def test_latest_failed_job_for_session_ignores_failure_before_completed_job(self) -> None:
        runner = mobile_portal.JobRunner(_FakeStore())
        runner.jobs["failed"] = {
            "job_id": "failed",
            "status": "failed",
        ...
        runner.jobs["completed"] = {
            ...
            "pid": 0,
            "error": "",
            "last_message": "new answer",
            "log_tail": [],
            "live_text": "new answer",
            "live_chunks_version": 0,
        }
    ...
    def test_run_resume_job_records_user_history_for_forked_session_with_job_created_timestamp(self) -> None:
        ...
        with mock.patch("mobile_portal.ensure_backend_proxy_ready"), \
        ...
        with mock.patch("mobile_portal.ensure_backend_proxy_ready"), \
                mock.patch.object(runner, "_run_codex_process", return_value="session-2"), \
                mock.patch.object(runner.data_store, "append_history_entry") as append_history_entry:
            runner._run_resume_job(
                "job-1",
                str(Path.cwd()),
    ...
    def test_run_codex_process_ignores_empty_task_complete_until_final_answer(self) -> None:
        runner = mobile_portal.JobRunner(_FakeStore())
        runner.jobs["job-1"] = {
            "job_id": "job-1",
            "status": "running",
        ...
        with mock.patch.object(mobile_portal, "PROCESS_EXIT_GRACE_SECONDS", 0.05), \
            ...
            worker.start()
            time.sleep(0.2)
            self.assertTrue(worker.is_alive(), "empty task_complete must not finish the process")
            self.assertEqual(0, fake_process.terminate_calls)
            fake_process.stdout.put(
                '{"type":"response_item","payload":{"type":"message","role":"assistant","phase":"final_answer","content":[{"text":"actual final answer"}]}}\n'
            )
        ...
        job = runner.get_job("job-1")
        self.assertEqual("actual final answer", job["last_message"])
        self.assertEqual(0, fake_process.terminate_calls)
        self.assertEqual(0, fake_process.kill_calls)

    def test_run_codex_process_rejects_empty_task_complete_after_commentary(self) -> None:
        runner = mobile_portal.JobRunner(_FakeStore())
        runner.jobs["job-1"] = {
            "job_id": "job-1",
            "status": "running",
        ...
        with mock.patch("mobile_portal.subprocess.Popen", return_value=_FakeProcess()):
            with self.assertRaisesRegex(RuntimeError, "without a final answer"):
...
class PortalServiceBootstrapTests(unittest.TestCase):
    ...
    def test_session_payload_hides_incomplete_placeholder_while_active_job_is_running(self) -> None:
        session = mobile_portal.SessionItem(
            session_id="session-1",
            ts=1,
            text="hello",
        ...
        fake_store = _FakeStore([session])
        fake_store.session_payload = mock.Mock(
            return_value={
                "session": mobile_portal.asdict(session),
                "messages": [
                    {"role": "user", "text": "start now?"},
                    {"role": "assistant", "text": mobile_portal.INCOMPLETE_REPLY_PLACEHOLDER},
                ],
            }
        )
        service = mobile_portal.PortalService("127.0.0.1", 8765, "token")
        ...
        payload = service.session_payload("session-1")

        self.assertIsNotNone(payload)
        self.assertEqual(
            [{"role": "user", "text": "start now?"}],
            [{"role": item["role"], "text": item["text"]} for item in payload["messages"]],
        )
    ...
    def test_startup_url_groups_include_configured_public_urls(self) -> None:
        ...
        self.assertEqual(
            [
                ("Public (Cloudflare/custom)", ["https://codex.example.com/?token=verify-token"]),
    ...
    def test_bootstrap_payload_includes_startup_url_groups(self) -> None:
        ...
        with mock.patch.object(
            service,
            "startup_url_groups",
            return_value=[("Public (Cloudflare/custom)", ["https://codex.example.com/?token=token"])],
        ):
            payload = service.bootstrap_payload()

        self.assertEqual(
            [{"label": "Public (Cloudflare/custom)", "urls": ["https://codex.example.com/?token=token"]}],
    ...
    def test_session_payload_includes_recent_failed_job_for_error_display(self) -> None:
        session = mobile_portal.SessionItem(
            session_id="session-1",
            ts=1,
            text="hello",
        ...
        service.jobs.jobs["job-1"] = {
            ...
            "finished_at": 2,
            "pid": 0,
            "error": "Codex exited without a final answer.",
            "last_message": "",
            "log_tail": [],
            "live_text": "",
            "live_chunks_version": 0,
        ...
        self.assertIsNotNone(payload)
        self.assertFalse(payload["session"]["is_replying"])
        self.assertEqual("failed", payload["active_job"]["status"])
        self.assertEqual("Codex exited without a final answer.", payload["active_job"]["error"])
    ...
    def test_proxy_settings_payload_and_update_round_trip(self) -> None:
        ...
        self.assertEqual(["https://codex.example.com/"], initial["public_urls"])
        ...
        self.assertEqual(["https://codex.example.com/"], updated["public_urls"])
...
class PortalPageTemplateTests(unittest.TestCase):
    ...
    def test_backend_panel_exposes_openai_preset_controls(self) -> None:
        html = mobile_portal.INDEX_HTML

        self.assertIn('data-tab="backend"', html)
        self.assertIn('id="backendPresetSelect"', html)
        self.assertIn('id="backendPresetName"', html)
        self.assertIn('id="backendProxyPreference"', html)
        self.assertIn('id="backendOpenAiApiKey" type="text"', html)
        self.assertIn('id="saveBackendPreset"', html)
        self.assertIn('id="applyBackendPreset"', html)
        self.assertIn('id="deleteBackendPreset"', html)
        self.assertNotIn('id="backendPresetId"', html)
        self.assertNotIn('id="backendOpenAiModel"', html)
        self.assertIn("refreshBackendPanel", html)
        self.assertIn("saveBackendPreset", html)
        self.assertIn("applyBackendPreset", html)
        self.assertIn("deleteBackendPreset", html)

    def test_mobile_portal_uses_operational_shell_not_hero_card(self) -> None:
        html = mobile_portal.INDEX_HTML
    ...
    def test_mobile_portal_keeps_required_controls_after_refresh(self) -> None:
        html = mobile_portal.INDEX_HTML
...
class PortalAccountSlotsTests(unittest.TestCase):
    ...
    def test_restart_remote_computer_invokes_remote_ssh(self) -> None:
        service = mobile_portal.PortalService("127.0.0.1", 8765, "token")
        completed = subprocess.CompletedProcess(["ssh"], 0, stdout="sent")

        with mock.patch.object(mobile_portal.remote_ssh, "restart_computer", return_value=completed) as restart_computer:
            payload = service.restart_remote_computer(
                host="100.64.0.10",
                user="codexuser",
        ...
        self.assertEqual({"ok": True, "host": "100.64.0.10", "user": "codexuser", "output": "sent"}, payload)
        restart_computer.assert_called_once_with(
            user="codexuser",
            host="100.64.0.10",
            identity_file=r"C:\Users\codexuser\.ssh\id_ed25519_remote",
            password="",
    ...
    def test_restart_remote_computer_raises_on_ssh_failure(self) -> None:
        service = mobile_portal.PortalService("127.0.0.1", 8765, "token")
        completed = subprocess.CompletedProcess(["ssh"], 255, stdout="permission denied")

        with mock.patch.object(mobile_portal.remote_ssh, "restart_computer", return_value=completed):
            with self.assertRaisesRegex(RuntimeError, "permission denied"):
                service.restart_remote_computer(host="100.64.0.10", user="codexuser")

    def test_restart_remote_computer_uses_configured_default_target(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            service = mobile_portal.PortalService("127.0.0.1", 8765, "token")
            service.proxy_settings_file = settings_file
            completed = subprocess.CompletedProcess(["plink"], 0, stdout="sent")

            with mock.patch.object(mobile_portal.remote_ssh, "restart_computer", return_value=completed) as restart_computer:
                payload = service.restart_remote_computer(host="", user="", password="example-password")

        self.assertEqual("100.64.0.10", payload["host"])
        self.assertEqual("codexuser", payload["user"])
        restart_computer.assert_called_once_with(
            user="codexuser",
            host="100.64.0.10",
            identity_file="",
    ...
    def test_restart_local_computer_invokes_shutdown(self) -> None:
        service = mobile_portal.PortalService("127.0.0.1", 8765, "token")
        completed = subprocess.CompletedProcess(["shutdown"], 0, stdout="")

        with mock.patch.object(mobile_portal.subprocess, "run", return_value=completed) as run:
            payload = service.restart_local_computer()

        self.assertEqual({"ok": True, "target": "local", "output": ""}, payload)
        run.assert_called_once()
        self.assertEqual(["shutdown", "/r", "/t", "0"], run.call_args.args[0])
    ...
    def test_read_current_weekly_quota_labels_single_weekly_primary_window(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            auth_path = Path(temp_dir) / "auth.json"
            auth_path.write_text(json.dumps({"tokens": {"access_token": "test-token"}}), encoding="utf-8")
            settings_path = Path(temp_dir) / "mobile_portal_settings.json"
        ...
        self.assertEqual("ok", quota["state"])
        self.assertEqual("Weekly quota: 100% used (resets in 6d 17h)", quota["summary"])
    ...
    def test_backend_status_payload_includes_current_token_quota_for_single_token_pool_file(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            service = mobile_portal.PortalService("127.0.0.1", 8765, "token")
            backend_settings_path = Path(temp_dir) / "token_pool_settings.json"
            token_dir = Path(temp_dir) / "tokens"
            token_dir.mkdir()
            (token_dir / "only.json").write_text(
                json.dumps(
                    {
            ...
            with mock.patch.object(
                mobile_portal,
                "token_pool_proxy_is_healthy",
                return_value={"status": "ok", "token_count": 1, "port": 8317, "current_token_file": ""},
            ), mock.patch.object(
                mobile_portal,
                "read_token_pool_token_quota",
                return_value={
                    "state": "ok",
                    "summary": "5h quota: 4% used\nWeekly quota: 16% used",
                    "email": "single@example.com",
        ...
        read_quota.assert_called_once_with(token_dir / "only.json", settings_file=service.proxy_settings_file)
        self.assertEqual("ok", payload["current_token_quota"]["state"])
        self.assertEqual("only.json", payload["current_token_quota"]["token_file"])
    ...
    def test_backend_status_payload_reports_openai_compatible_runtime(self) -> None:
        ...
        self.assertEqual("default", payload["openai_presets"][0]["id"])
        self.assertEqual("Default", payload["openai_presets"][0]["name"])
        self.assertEqual("https://api.openai.com/v1", payload["openai_presets"][0]["openai_base_url"])
        self.assertEqual("direct", payload["openai_presets"][0]["proxy_preference"])
        self.assertEqual("http://127.0.0.1:7898", payload["upstream_proxy_url"])
        self.assertEqual("http://127.0.0.1:7898", payload["openai_presets"][0]["upstream_proxy_url"])
        self.assertEqual("sk-test", payload["openai_api_key"])
        self.assertEqual("sk-test", payload["openai_presets"][0]["openai_api_key"])
        self.assertTrue(payload["has_openai_api_key"])
        self.assertTrue(payload["openai_presets"][0]["has_openai_api_key"])
    ...
    def test_update_backend_settings_does_not_mutate_codex_config_for_openai_compatible(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with mock.patch.object(mobile_portal, "_CODEX_CONFIG_PATH", config_path), mock.patch.object(
                token_pool_settings,
                "resolve_openai_compatible_backend_config",
                return_value={
                    "openai_base_url": "https://api.openai.com/v1",
    ...
    def test_update_backend_settings_can_save_named_openai_preset(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with mock.patch.object(
                ...
                "resolve_openai_compatible_backend_config",
                return_value={
                    "openai_base_url": "https://gptcode.top/v1",
                    "openai_api_key": "sk-test",
                    "openai_model": "gpt-5.4",
                    "openai_models": ["gpt-5.4", "gpt-5.5"],
                    "openai_protocol": "chat_completions",
                },
            ):
                updated = service.update_backend_settings(
        ...
        self.assertEqual("gptcode", updated["active_openai_preset_id"])
        self.assertEqual("gptcode", loaded["active_openai_preset_id"])
        self.assertEqual("https://gptcode.top/v1", loaded["openai_base_url"])
        self.assertEqual("chat_completions", loaded["openai_protocol"])
        self.assertEqual("http://127.0.0.1:7898", loaded["upstream_proxy_url"])
        presets = {item["id"]: item for item in updated["openai_presets"]}
        self.assertEqual("GPTCode", presets["gptcode"]["name"])
        self.assertEqual("https://gptcode.top/v1", presets["gptcode"]["openai_base_url"])
        self.assertEqual("direct", presets["gptcode"]["proxy_preference"])
        self.assertEqual("http://127.0.0.1:7898", presets["gptcode"]["upstream_proxy_url"])

    def test_update_backend_settings_uses_existing_preset_url_when_only_key_is_changed(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            service = mobile_portal.PortalService("127.0.0.1", 8765, "token")
            backend_settings_path = Path(temp_dir) / "token_pool_settings.json"
            token_dir = Path(temp_dir) / "tokens"
            ...
            token_pool_settings.save_backend_settings(
                ...
                proxy_port=8456,
                proxy_api_key="pool-api-key",
                openai_base_url="https://wrong-current.example/v1",
                openai_api_key="sk-old-current",
            ...
            token_pool_settings.save_openai_preset(
                ...
                preset_id="daily",
                name="Daily",
                openai_base_url="https://daily.example/v1",
                openai_api_key="sk-old-daily",
                openai_model="gpt-5.5",
                openai_models=["gpt-5.5"],
                openai_protocol="responses",
                set_active=False,
        ...
        resolve_backend.assert_called_once_with(
            "https://daily.example/v1",
            "sk-new-daily",
            "gpt-5.5",
        )
        self.assertEqual("https://daily.example/v1", loaded["openai_base_url"])
        self.assertEqual("https://daily.example/v1", updated["openai_base_url"])
        presets = {item["id"]: item for item in loaded["openai_presets"]}
        self.assertEqual("https://daily.example/v1", presets["daily"]["openai_base_url"])

    def test_update_backend_settings_stops_openai_proxy_when_responses_mode_changes(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            service = mobile_portal.PortalService("127.0.0.1", 8765, "token")
            backend_settings_path = Path(temp_dir) / "token_pool_settings.json"
            token_dir = Path(temp_dir) / "tokens"
            ...
            ), mock.patch.object(mobile_portal, "stop_token_pool_backend") as stop_backend, \
                 mock.patch.object(mobile_portal, "start_openai_compatible_backend", return_value={"status": "ok"}) as start_backend:
                service.update_backend_settings(
                    backend_mode=token_pool_settings.BACKEND_MODE_OPENAI_COMPATIBLE,
                    token_dir=str(token_dir),
                    proxy_port=8456,
    ...
    def test_update_backend_settings_derives_openai_preset_id_from_name_and_detects_model(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            service = mobile_portal.PortalService("127.0.0.1", 8765, "token")
            backend_settings_path = Path(temp_dir) / "token_pool_settings.json"
            token_dir = Path(temp_dir) / "tokens"
            ...
            with mock.patch.object(
                ...
                return_value={
                    ...
                    "openai_api_key": "sk-test",
                    "openai_model": "detected-model",
                    "openai_models": ["detected-model", "other-model"],
                    "openai_protocol": "responses",
                },
            ) as resolve_backend:
                updated = service.update_backend_settings(
        ...
        resolve_backend.assert_called_once_with("https://api.example.com/v1", "sk-test", "")
        self.assertEqual("local-grok", updated["active_openai_preset_id"])
        self.assertEqual("detected-model", loaded["openai_model"])
        presets = {item["id"]: item for item in updated["openai_presets"]}
        self.assertEqual("Local Grok", presets["local-grok"]["name"])
        self.assertEqual("detected-model", presets["local-grok"]["openai_model"])

    def test_apply_and_delete_responses_openai_backend_preset_do_not_restart_openai_proxy(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            service = mobile_portal.PortalService("127.0.0.1", 8765, "token")
            backend_settings_path = Path(temp_dir) / "token_pool_settings.json"
            service.backend_settings_file = backend_settings_path
            ...
            with mock.patch.object(
                ...
                return_value={
                 ...
                 mock.patch.object(mobile_portal, "stop_token_pool_backend") as stop_backend, \
                 mock.patch.object(mobile_portal, "start_openai_compatible_backend", return_value={"status": "ok"}) as start_backend:
                applied = service.apply_openai_backend_preset("gptcode")
                deleted = service.delete_openai_backend_preset("gptcode")

            loaded = token_pool_settings.load_backend_settings(backend_settings_path)

        self.assertEqual("gptcode", applied["active_openai_preset_id"])
        self.assertEqual("responses", applied["openai_protocol"])
        resolve_backend.assert_called_once_with("https://gptcode.top/v1", "sk-test", "gpt-5.4", upstream_proxy_url="")
        self.assertEqual("default", deleted["active_openai_preset_id"])
        self.assertEqual(["default"], [item["id"] for item in loaded["openai_presets"]])
        self.assertEqual(2, stop_backend.call_count)
        start_backend.assert_not_called()

    def test_apply_openai_backend_preset_skips_validation_from_preset_metadata(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            service = mobile_portal.PortalService("127.0.0.1", 8765, "token")
            backend_settings_path = Path(temp_dir) / "token_pool_settings.json"
            service.backend_settings_file = backend_settings_path
            ...
            with mock.patch.object(token_pool_settings, "resolve_openai_compatible_backend_config") as resolve_backend, \
                 mock.patch.object(mobile_portal, "stop_token_pool_backend"), \
                 mock.patch.object(mobile_portal, "start_openai_compatible_backend", return_value={"status": "ok"}):
                applied = service.apply_openai_backend_preset("private-provider")

        resolve_backend.assert_not_called()
        self.assertEqual("private-provider", applied["active_openai_preset_id"])
        self.assertEqual("private-model", applied["openai_model"])
        self.assertEqual("responses", applied["openai_protocol"])

    def test_apply_openai_backend_preset_persists_skip_validation_overrides(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            service = mobile_portal.PortalService("127.0.0.1", 8765, "token")
            backend_settings_path = Path(temp_dir) / "token_pool_settings.json"
            service.backend_settings_file = backend_settings_path
            ...
            with mock.patch.object(token_pool_settings, "resolve_openai_compatible_backend_config") as resolve_backend, \
                 mock.patch.object(mobile_portal, "stop_token_pool_backend"), \
                 mock.patch.object(mobile_portal, "start_openai_compatible_backend", return_value={"status": "ok"}):
                applied = service.apply_openai_backend_preset(
                    "private-provider",
        ...
        resolve_backend.assert_not_called()
        self.assertEqual("private-provider", applied["active_openai_preset_id"])
        self.assertEqual("private-alt", applied["openai_model"])
        self.assertEqual("chat_completions", applied["openai_protocol"])
        preset = next(item for item in loaded["openai_presets"] if item["id"] == "private-provider")
    ...
    def test_update_backend_settings_skips_validation_for_skip_preset(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            service = mobile_portal.PortalService("127.0.0.1", 8765, "token")
            backend_settings_path = Path(temp_dir) / "token_pool_settings.json"
            token_dir = Path(temp_dir) / "tokens"
        ...
        self.assertEqual("private-alt", loaded["openai_model"])
        self.assertEqual("chat_completions", loaded["openai_protocol"])
        preset = next(item for item in loaded["openai_presets"] if item["id"] == "private-provider")
        self.assertEqual("private-alt", preset["openai_model"])
    ...
    def test_update_backend_settings_uses_selected_preset_key_when_api_key_blank(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            service = mobile_portal.PortalService("127.0.0.1", 8765, "token")
            backend_settings_path = Path(temp_dir) / "token_pool_settings.json"
            service.backend_settings_file = backend_settings_path
            ...
            with mock.patch.object(
                ...
                return_value={
                    ...
                    "openai_api_key": "sk-second",
                    "openai_model": "second-model",
                    "openai_models": ["second-model", "second-alt"],
                    "openai_protocol": "responses",
                },
            ) as resolve_backend:
                updated = service.update_backend_settings(
        ...
        resolve_backend.assert_called_once_with("https://second.example/v1", "sk-second", "second-model")
        self.assertEqual("second", updated["active_openai_preset_id"])
        self.assertEqual("sk-second", loaded["openai_api_key"])
        self.assertEqual("sk-second", next(
            item for item in loaded["openai_presets"] if item["id"] == "second"
        )["openai_api_key"])
...
class PortalFileShareTests(unittest.TestCase):
    def test_build_inline_content_disposition_supports_utf8_pdf_names(self) -> None:
        filename = "\u52a8\u91cf\u5b88\u6052.pdf"
        header = mobile_portal.build_inline_content_disposition(filename)

        self.assertIn('inline; filename="', header)
        self.assertIn("filename*=UTF-8''", header)
        self.assertIn("%E5%8A%A8%E9%87%8F%E5%AE%88%E6%81%92", header)
        self.assertNotIn(f'filename="{filename}"', header)
...
class PortalDownloadsTests(unittest.TestCase):
    def test_download_page_points_to_github_releases_without_local_artifacts(self) -> None:
        service = mobile_portal.PortalService("127.0.0.1", 8765, "verify-token")

        html = service.download_page_html()

        self.assertIn("https://github.com/penguin-oo/codex-session-manager-windows/releases", html)
        self.assertIn("Packaged ZIP/APK files are not served", html)
        self.assertNotIn("/downloads/codex-mobile-debug.apk", html)
        self.assertNotIn("/downloads/codex-session-manager-windows-x64.zip", html)
...
class PortalAuthorizationTests(unittest.TestCase):
    ...
    def test_is_authorized_accepts_exact_non_ascii_header_token(self) -> None:
        token = "\u4ee4\u724c-\u6d4b\u8bd5"
        handler = self._make_handler(portal_token=token, header_token=token)

        self.assertTrue(handler._is_authorized())

    def test_is_authorized_accepts_exact_non_ascii_query_token(self) -> None:
        token = "\u4ee4\u724c-\u6d4b\u8bd5"
        handler = self._make_handler(portal_token=token, path="/api/bootstrap?token=%E4%BB%A4%E7%89%8C-%E6%B5%8B%E8%AF%95")
...
class PortalBrowserControlTests(unittest.TestCase):
    ...
    def test_do_post_backend_dispatches_openai_compatible_fields(self) -> None:
        ...
        portal.update_backend_settings.assert_called_once_with(
            ...
            openai_protocol="",
            openai_manual_extra_models=None,
            proxy_preference="",
            upstream_proxy_url="http://127.0.0.1:7898",
        )
        send_json.assert_called_once_with({"backend_mode": "openai_compatible"})

    def test_do_post_backend_dispatches_create_new_preset_flag(self) -> None:
        portal = SimpleNamespace(
            token="token",
            update_backend_settings=mock.Mock(return_value={"active_openai_preset_id": "provider-2"}),
        )
        handler = self._make_handler(path="/api/backend?token=token", portal=portal)
        ...
        self.assertFalse(portal.update_backend_settings.call_args.kwargs["create_new_preset"])
        self.assertEqual("provider", portal.update_backend_settings.call_args.kwargs["preset_id"])
        self.assertEqual("Provider", portal.update_backend_settings.call_args.kwargs["preset_name"])
        self.assertEqual("proxy", portal.update_backend_settings.call_args.kwargs["proxy_preference"])
        send_json.assert_called_once_with({"active_openai_preset_id": "provider-2"})

    def test_do_post_remote_restart_dispatches_action(self) -> None:
        portal = SimpleNamespace(
            token="token",
            restart_remote_computer=mock.Mock(return_value={"ok": True}),
        )
        handler = self._make_handler(path="/api/remote/restart?token=token", portal=portal)
        ...
        portal.restart_remote_computer.assert_called_once_with(
            host="100.64.0.10",
            user="codexuser",
            identity_file=r"C:\Users\codexuser\.ssh\id_ed25519_remote",
            password="example-password",
        ...
        send_json.assert_called_once_with({"ok": True}, status=mobile_portal.HTTPStatus.ACCEPTED)

    def test_do_post_desktop_restart_dispatches_action(self) -> None:
        portal = SimpleNamespace(
            token="token",
            restart_local_computer=mock.Mock(return_value={"ok": True, "target": "local"}),
        )
        handler = self._make_handler(path="/api/desktop/restart?token=token", portal=portal)
        ...
        portal.restart_local_computer.assert_called_once_with()
        send_json.assert_called_once_with({"ok": True, "target": "local"}, status=mobile_portal.HTTPStatus.ACCEPTED)
...
class ResumeArgsTests(unittest.TestCase):
    ...
    def test_build_resume_args_codex_auth_resolves_default_to_primary_model(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            backend_settings_path = Path(temp_dir) / "token_pool_settings.json"
            token_pool_settings.save_backend_settings(
                token_pool_settings.BACKEND_MODE_CODEX_AUTH,
        ...
        self.assertEqual(mobile_portal.DEFAULT_PRIMARY_MODEL, args[args.index("-m") + 1])
...
class ProxyEnvTests(unittest.TestCase):
    def test_build_codex_subprocess_env_defaults_to_local_proxy(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with mock.patch.dict(mobile_portal.os.environ, {}, clear=True):
                env = mobile_portal.build_codex_subprocess_env(backend_settings_file=backend_settings_path)
    ...
    def test_build_codex_subprocess_env_uses_upstream_key_for_openai_responses(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            proxy_settings_path = Path(temp_dir) / "mobile_portal_settings.json"
            backend_settings_path = Path(temp_dir) / "token_pool_settings.json"
            mobile_portal.save_proxy_settings(True, 9003, proxy_settings_path)
    ...
    def test_build_codex_subprocess_env_uses_local_proxy_key_for_openai_chat_completions(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            proxy_settings_path = Path(temp_dir) / "mobile_portal_settings.json"
            backend_settings_path = Path(temp_dir) / "token_pool_settings.json"
            mobile_portal.save_proxy_settings(True, 9003, proxy_settings_path)
    ...
    def test_build_codex_subprocess_env_removes_api_keys_for_codex_auth_mode(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            proxy_settings_path = Path(temp_dir) / "mobile_portal_settings.json"
            backend_settings_path = Path(temp_dir) / "token_pool_settings.json"
            mobile_portal.save_proxy_settings(True, 9003, proxy_settings_path)
            ...
            base_env = {
                "CODEX_API_KEY": "stale-local-proxy-key",
                "OPENAI_API_KEY": "stale-openai-key",
                "CODEX_TOKEN_POOL_API_KEY": "stale-token-pool-key",
                "CODEX_OPENAI_COMPATIBLE_API_KEY": "stale-compatible-key",
            }
        ...
        self.assertNotIn("CODEX_TOKEN_POOL_API_KEY", env)
        self.assertNotIn("CODEX_OPENAI_COMPATIBLE_API_KEY", env)
...
class TokenPoolBackendStartupTests(unittest.TestCase):
    ...
    def test_openai_compatible_proxy_health_requires_matching_config_fingerprint(self) -> None:
        ...
        self.assertTrue(mobile_portal.openai_compatible_proxy_health_matches_settings(matching_health, settings))
        self.assertFalse(
            mobile_portal.openai_compatible_proxy_health_matches_settings(
    ...
    def test_build_token_pool_proxy_command_uses_py_launcher_when_python_is_windowsapps_shim(self) -> None:
        ...
        with mock.patch.object(mobile_portal.shutil, "which", side_effect=fake_which):
            command = mobile_portal.build_token_pool_proxy_command(
        ...
        self.assertEqual(["C:\\Windows\\py.exe", "-3", "D:\\codex\\manger\\mobile_portal.py"], command[:3])
    ...
    def test_build_custom_provider_proxy_command_uses_py_launcher_when_python_is_windowsapps_shim(self) -> None:
        ...
        with mock.patch.object(mobile_portal.shutil, "which", side_effect=fake_which):
            command = mobile_portal.build_custom_provider_proxy_command(
                executable=executable,
                app_path="D:\\codex\\manger\\mobile_portal.py",
                port=8317,
    ...
    def test_build_arg_parser_accepts_custom_provider_proxy_mode(self) -> None:
        ...
        self.assertEqual("test-upstream-key", args.upstream_api_key)
        self.assertEqual("responses", args.upstream_protocol)
        self.assertEqual(["mimo-v2-pro"], args.model)

    def test_build_arg_parser_accepts_custom_provider_chat_completions_mode(self) -> None:
        args = mobile_portal.build_arg_parser().parse_args(
            [
                "--custom-provider-proxy",
                "--port",
...
class BackendOverrideArgsTests(unittest.TestCase):
    ...
    def test_build_backend_override_args_points_responses_protocol_to_upstream_url(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            backend_settings_path = Path(temp_dir) / "token_pool_settings.json"
            token_pool_settings.save_backend_settings(
                token_pool_settings.BACKEND_MODE_OPENAI_COMPATIBLE,
    ...
    def test_build_backend_override_args_points_chat_protocol_to_local_proxy(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            backend_settings_path = Path(temp_dir) / "token_pool_settings.json"
            token_pool_settings.save_backend_settings(
                token_pool_settings.BACKEND_MODE_OPENAI_COMPATIBLE,
...
class LoadMessagesTests(unittest.TestCase):
    def test_primary_session_jsonl_name_rejects_recovery_artifacts(self) -> None:
        self.assertTrue(mobile_portal.is_primary_session_jsonl_name("rollout-session-1.jsonl"))
        self.assertFalse(mobile_portal.is_primary_session_jsonl_name("rollout-session-1.jsonl.context-overflow-backup-20260507-010101.jsonl"))
        self.assertFalse(mobile_portal.is_primary_session_jsonl_name("rollout-session-1.jsonl.restore-current-backup-20260507-010101.jsonl"))
        self.assertFalse(mobile_portal.is_primary_session_jsonl_name("rollout-session-1.jsonl.merged-restore-candidate-20260507-010101.jsonl"))
        self.assertFalse(mobile_portal.is_primary_session_jsonl_name("rollout-session-1.jsonl.full-restored-archive-20260507-010101.jsonl"))
        self.assertFalse(mobile_portal.is_primary_session_jsonl_name("rollout-session-1.jsonl.goal-clear-backup-20260618-010101.jsonl"))
    ...
    def test_load_messages_ignores_turn_aborted_internal_messages(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir)
            history_file = temp_root / "history.jsonl"
            sessions_dir = temp_root / "sessions" / "2026" / "03" / "19"
    ...
    def test_load_messages_ignores_goal_internal_context_messages(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir)
            history_file = temp_root / "history.jsonl"
            sessions_dir = temp_root / "sessions" / "2026" / "03" / "19"
                        ...
                        mobile_portal.json.dumps(
                            {
                                ...
                                "payload": {
                                    "type": "message",
                                    "role": "user",
                                    "content": [{"type": "input_text", "text": "start now?"}],
                                },
                            },
                            ensure_ascii=False,
                        ),
    ...
    def test_clear_session_goal_context_removes_goal_context_and_writes_backup(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir)
            history_file = temp_root / "history.jsonl"
            sessions_dir = temp_root / "sessions" / "2026" / "03" / "19"
            ...
            with mock.patch.object(mobile_portal, "HISTORY_FILE", history_file), \
                    mock.patch.object(mobile_portal, "SESSIONS_DIR", temp_root / "sessions"):
                store = mobile_portal.CodexDataStore()
                result = store.clear_session_goal_context("session-1")

            self.assertEqual({"ok": True, "session_id": "session-1", "removed": 1}, {key: result[key] for key in ("ok", "session_id", "removed")})
            backup_path = Path(str(result["backup_file"]))
            self.assertTrue(backup_path.exists())
            self.assertIn("codex_internal_context", backup_path.read_text(encoding="utf-8"))
            cleaned_text = session_file.read_text(encoding="utf-8")
            self.assertNotIn("codex_internal_context", cleaned_text)
            self.assertIn("normal prompt", cleaned_text)

    def test_portal_service_clear_session_goal_context_delegates_to_data_store(self) -> None:
        fake_store = _FakeStore()
        fake_store.clear_session_goal_context = mock.Mock(return_value={"ok": True, "session_id": "session-1", "removed": 1})
        service = mobile_portal.PortalService("127.0.0.1", 8765, "token")
        service.data_store = fake_store

        result = service.clear_session_goal_context("session-1")

        self.assertEqual({"ok": True, "session_id": "session-1", "removed": 1}, result)
        fake_store.clear_session_goal_context.assert_called_once_with("session-1")

    def test_load_messages_ignores_handoff_summary_context_messages(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir)
            history_file = temp_root / "history.jsonl"
            sessions_dir = temp_root / "sessions" / "2026" / "03" / "19"
                        ...
                        mobile_portal.json.dumps(
                            {
                                ...
                                "payload": {
                                    ...
                                    "content": [
                                        {
                                            "type": "input_text",
                                            "text": "**Handoff Summary**\n\n**Current Task**\nInternal state that must not be shown.",
                                        }
                                    ],
                                },
                            },
    ...
    def test_load_messages_omits_placeholder_when_task_complete_is_null(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir)
            history_file = temp_root / "history.jsonl"
            sessions_dir = temp_root / "sessions" / "2026" / "03" / "19"
    ...
    def test_load_messages_keeps_task_complete_fallback_when_user_message_already_exists_in_history(self) -> None:
                        ...
                        mobile_portal.json.dumps(
                            {
                                ...
                                "payload": {
                                    ...
                                    "role": "assistant",
                                    "phase": "commentary",
                                    "content": [{"type": "output_text", "text": "not stuck, smoke test passed"}],
                                },
                            },
                            ensure_ascii=False,
                        ),
                        ...
                        mobile_portal.json.dumps(
                            {
                                ...
                                "payload": {
                                    "type": "task_complete",
                                    "turn_id": "turn-1",
                                    "last_agent_message": "not stuck, smoke test passed",
                                },
                            },
                            ensure_ascii=False,
                        ),
        ...
        self.assertEqual(
            [
                ...
                {"role": "assistant", "text": "not stuck, smoke test passed"},
            ],
            [{"role": item["role"], "text": item["text"]} for item in messages],
        )
