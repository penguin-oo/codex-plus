...
class TokenPoolSettingsTests(unittest.TestCase):
    ...
    def test_import_token_files_expands_sub2api_payload(self) -> None:
        source_dir = self.root / 'source'
        source_dir.mkdir()
        source = source_dir / 'sub2api_import.json'
        source.write_text(
            json.dumps(
                {
        ...
        imported = token_pool_settings.import_token_files([source], token_dir=self.token_dir)
        ...
        self.assertEqual(['access-alpha', 'access-beta'], [payload['access_token'] for payload in imported_payloads])
        self.assertEqual(['refresh-alpha', 'refresh-beta'], [payload['refresh_token'] for payload in imported_payloads])
        self.assertEqual(['acct-alpha', 'acct-beta'], [payload['account_id'] for payload in imported_payloads])
        self.assertEqual(['sub2api', 'sub2api'], [payload['source_format'] for payload in imported_payloads])
...
class TokenPoolCoreTests(unittest.TestCase):
    ...
    def test_select_token_reads_accounts_from_direct_sub2api_payload(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            token_dir = Path(temp_dir)
            (token_dir / 'sub2api_import.json').write_text(
                json.dumps(
                    {
                        'accounts': {
                            'alpha': {
            ...
            second = pool.select_token()

        self.assertEqual('access-alpha', first.access_token)
        self.assertEqual('access-beta', second.access_token)
...
class TokenPoolAppTests(unittest.TestCase):
    ...
    def test_build_health_payload_reports_token_count_and_port(self) -> None:
        ...
        self.assertEqual('', payload['current_token_file'])
    ...
    def test_forward_responses_request_updates_current_token_file_after_success(self) -> None:
        response = self.app.forward_responses_request(
            auth_header='Bearer local-proxy-key',
            body_bytes=b'{"model":"gpt-5.4","input":"Hi"}',
            upstream_fn=lambda *_args, **_kwargs: token_pool_proxy.ForwardResponse(
                status_code=200,
                body=b'ok',
        ...
        self.assertEqual(200, response.status_code)
        self.assertEqual('a.json', self.app.build_health_payload()['current_token_file'])
