...
class AuthSlotsTests(unittest.TestCase):
    ...
    def test_update_account_slot_note_preserves_multiline_free_text(self) -> None:
        created = auth_slots.create_account_slot("Research", slots_dir=self.slots_dir)
        note = "first line\n第二行\n\npaste anything: https://example.test/?a=1&b=2"

        updated = auth_slots.update_account_slot_note(created["slot_id"], note, slots_dir=self.slots_dir)

        self.assertEqual(note, updated["note"])
        listed = auth_slots.list_account_slots(codex_home=self.codex_home, slots_dir=self.slots_dir)
        self.assertEqual(note, listed[0]["note"])
