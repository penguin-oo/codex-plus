...
def update_account_slot_note(slot_id: str, note: str, slots_dir: Path = ACCOUNT_SLOTS_DIR) -> dict[str, str]:
    items = load_slot_registry(slots_dir=slots_dir)
    for item in items:
        if item["slot_id"] == slot_id:
...
def get_slot_info(slot_id: str, slots_dir: Path = ACCOUNT_SLOTS_DIR) -> dict[str, str]:
    ...
    info["label"] = label
    info["note"] = next((item.get("note", "") for item in load_slot_registry(slots_dir=slots_dir) if item["slot_id"] == slot_id), "")
