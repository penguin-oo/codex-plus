...
class TokenPool:
    ...
    def _load_states(self) -> list[TokenState]:
        ...
        for path in sorted(self.token_dir.glob('*.json')):
            ...
            if not isinstance(payload, dict):
                continue
            sub2api_records = token_pool_settings.build_sub2api_token_file_records(path.name, payload)
            if sub2api_records:
                for file_name, token_payload in sub2api_records:
                    access_token = str(
                        token_payload.get('access_token')
                        or token_payload.get('token')
                        or token_payload.get('api_key')
                        or ''
                    ).strip()
                    if not access_token:
                        continue
                    states.append(TokenState(file_name=file_name, access_token=access_token, source_path=path))
                continue
            access_token = str(
    ...
    def state_for_last_selection(self) -> TokenState | None:
        if not self._last_selected_file_name:
            return None
        for state in self._states:
            if state.file_name == self._last_selected_file_name:
                return state
        return None
    ...
    def select_token(self) -> TokenState:
        ...
        for offset in range(total):
            ...
            self._last_selected_file_name = state.file_name
            return state
...
class TokenPoolProxyApp:
    ...
    def build_health_payload(self) -> dict[str, object]:
        return {
            ...
            'current_token_file': self.current_token_file,
        }
    ...
    def forward_responses_request(
        ...
        *,
        ...
        try:
            response = TokenPoolForwarder(self.pool).forward_with_failover(
                lambda token_state: upstream_fn(token_state, translated, path)
            )
            selected_state = self.pool.state_for_last_selection()
            self.current_token_file = selected_state.file_name if selected_state is not None else ''
            return response
